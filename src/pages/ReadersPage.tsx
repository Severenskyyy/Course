...
export function ReadersPage() {
  const { readers, loans, hasRole, addReader, updateReader, deleteReader } = useApp();
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      ...
      toast.success('✏️ Читатель обновлён', `${form.fio.trim()} — изменения сохранены`);
    } else {
      addReader({
      ...
      toast.success('👤 Читатель добавлен', `${form.fio.trim()} (${form.group.trim()}) зарегистрирован`);
    }
    setModalOpen(false);
    setEditingId(null);
  };
          ...
          <div className="flex gap-3 pt-4 border-t border-slate-700/50">
            ...
            {editingId && (
              <button
                type="button"
                onClick={() => {
                  const hasActive = loans.some(l => l.reader_id === editingId && !l.return_date);
                  if (hasActive) {
                    toast.error('Нельзя удалить читателя', 'У читателя есть активные выдачи — сначала оформите возврат');
                    return;
                  }
                  const reader = readers.find(r => r.reader_id === editingId);
                  deleteReader(editingId);
                  toast.warning('🗑 Читатель удалён', reader ? `${reader.fio} удалён из системы` : 'Читатель удалён');
                  handleCloseModal();
                }}
                className="px-4 py-2.5 text-sm font-medium rounded-xl text-red-400 hover:bg-red-500/20 transition-all"
              >
                Удалить
              </button>
            )}
