...
export function CatalogPage() {
  ...
  const handleReserve = (book: Book) => {
    ...
    if (result.success) {
      toast.success('🔖 Книга забронирована', `«${book.title}» — бронь действует 24 часа`);
    } else {
      toast.error('Не удалось забронировать', result.message);
    }
  ...
  const handleSaveBook = () => {
    ...
    if (editBookCode) {
      ...
      toast.success('✏️ Книга обновлена', `«${newBook.title.trim()}» — изменения сохранены`);
    } else {
      addBook({
      ...
      toast.success('📘 Книга добавлена', `«${newBook.title.trim()}» добавлена в каталог`);
    }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
            ...
            {hasRole('admin', 'librarian') && (
              <div className="flex gap-3 pt-4 border-t border-slate-700/50">
                ...
                <button type="button" onClick={() => {
                  ...
                  toast.warning('🗑 Книга удалена', `«${title}» исключена из каталога`);
                  setSelectedBook(null);
      ...
      <ConfirmDialog
        ...
        message={`Вы хотите забронировать книгу «${confirmReserve?.title}»? Бронь действует 24 часа, одновременно можно держать до 5 книг.`}
