...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const canReserveBook = (bookCode: string): { canReserve: boolean; reason?: string } => {
    ...
    // Проверяем cooldown (3 дня) — повторная бронь той же книги
    ...
    // Проверяем лимит активных бронирований (не более 5 одновременно)
    const activeByReader = reservations.filter(r =>
      r.reader_id === currentUser.readerId &&
      r.is_active &&
      new Date(r.expires_at) > now
    );
    if (activeByReader.length >= 5) {
      return { canReserve: false, reason: 'Лимит: одновременно можно держать до 5 броней' };
    }
